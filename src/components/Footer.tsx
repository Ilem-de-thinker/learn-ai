...
import { Sparkles, ArrowUpRight, Github, Twitter, Linkedin, Lock } from 'lucide-react';
    ...
    <footer className="bg-[#0D0D0E] border-t border-zinc-800 text-zinc-400 py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-10">
        ...
        <div className="space-y-3">
          ...
          <ul className="space-y-2 text-sm">
            <li>
              <button onClick={() => { setView('home'); window.scrollTo(0,0); }} className="hover:text-white transition-colors">
                Home Overview
              </button>
            </li>
            <li>
              <button onClick={() => { setView('register'); window.scrollTo(0,0); }} className="hover:text-white transition-colors">
                Course Registration
              </button>
            </li>
            <li>
              <button onClick={() => { setView('admin'); window.scrollTo(0,0); }} className="flex items-center gap-1.5 hover:text-white transition-colors">
                <Lock className="w-3 h-3 text-[#C9A227]" />
                Admin Dashboard
              </button>
            </li>
            <li>
              <button onClick={() => { setView('terms'); window.scrollTo(0,0); }} className="hover:text-white transition-colors">
                Terms & Conditions
              </button>
            </li>
          </ul>
      ...
      <div className="max-w-7xl mx-auto mt-12 pt-8 border-t border-zinc-800/60 flex flex-col sm:flex-row items-center justify-between text-xs text-zinc-500 gap-4">
        ...
        <div className="flex items-center gap-6">
          <button onClick={() => setView('terms')} className="hover:text-zinc-300">Privacy Policy</button>
          <button onClick={() => setView('terms')} className="hover:text-zinc-300">Terms of Service</button>
