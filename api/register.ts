...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!country || !state || !occupation || !source) return res.status(400).json({ error: 'All fields required.' });
    ...
    const record: RegistrationRecord = {
      ...
      fullName, email: email.toLowerCase(), phone, country, state, occupation, experience, source,
